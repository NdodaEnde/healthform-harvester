...
import CertificatePromotionDialog from "@/components/certificates/CertificatePromotionDialog";
...
const DocumentViewer = () => {
  ...
  // Patient record creation dialog state
...
const normalizeDataForValidation = (extractedData: any): any => {
  ...
  // Function to extract validation data for patient creation
  const prepareValidationDataForPromotion = (data: any): any => {
    console.log("Preparing validation data for promotion:", data);
    
    // Handle different data structures
    let sourceData = data;
    if (data?.structured_data) {
      sourceData = data.structured_data;
    }
    
    // Extract patient information
    const patient = sourceData?.patient || {};
    const examination = sourceData?.examination_results || {};
    const certification = sourceData?.certification || {};
    const restrictions = sourceData?.restrictions || {};
    
    // Determine fitness status
    let fitnessStatus = 'unknown';
    if (certification.fit) fitnessStatus = 'fit';
    else if (certification.fit_with_restrictions) fitnessStatus = 'fit with restrictions';
    else if (certification.fit_with_condition) fitnessStatus = 'fit with condition';
    else if (certification.temporarily_unfit) fitnessStatus = 'temporarily unfit';
    ...
    let examinationType = 'unknown';
    if (examination.type?.pre_employment) examinationType = 'pre-employment';
    else if (examination.type?.periodical) examinationType = 'periodical';
    else if (examination.type?.exit) examinationType = 'exit';
    
    return {
      patientName: patient.name || 'Unknown',
      patientId: patient.id_number || patient.employee_id || patient.id || 'Unknown',
      companyName: patient.company || '',
      occupation: patient.occupation || '',
      fitnessStatus: fitnessStatus,
      restrictionsText: activeRestrictions || 'None',
      examinationDate: examination.date || certification.examination_date || '',
      expiryDate: certification.valid_until || '',
      examinationType: examinationType,
      comments: certification.comments || '',
      followUpActions: certification.follow_up || ''
    };
  };

  const handleValidateAndCreateRecord = async () => {
    if (!editableData || !currentOrganization) {
      toast.error("Missing required data for patient record creation");
      return;
    }
    
    try {
      // First save the validation
      await handleSaveValidation();
      
      // Then prepare data for patient creation
      const validationData = prepareValidationDataForPromotion(editableData);
      ...
      console.error('Error preparing validation data:', error);
      toast.error("Failed to prepare data for patient record creation");
    }
  };
  ...
  const handleSaveValidation = async () => {
    ...
    try {
      console.log('Saving validation:', editableData);
      ...
      // Add validation metadata for validation mode
      if (isValidating) {
        dataToSave = {
          ...editableData,
          validation_metadata: {
      ...
      console.error('Exception saving validation:', error);
  ...
  const handlePromotionComplete = () => {
    ...
    toast.success("Patient record created successfully!", {
      description: "The validated document data has been saved and a patient record has been created."
    });
  };
  ...
  const renderStructuredSection = (title: string, data: any, path: string[]) => {
    ...
    return (
      ...
      {/* Patient Record Creation Dialog */}
      {isPromotionDialogOpen && editableData && currentOrganization && (
        <CertificatePromotionDialog
          ...
          validatedData={prepareValidationDataForPromotion(editableData)}
            ...
            {isValidating ? (
              ...
              <Button onClick={async () => {
                try {
                  ...
                  const successMessage = "Your edits have been saved and will contribute to accuracy metrics.";
    ...
    <div className="min-h-screen flex flex-col">
      <header className="sticky top-0 z-40 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container flex h-16 items-center space-x-4">
          ...
          <div className="flex-1"> || isValidating) ? (
                  <Input 
                    className="border-b border-gray-400 flex-1 bg-transparent p-0 h-6 focus-visible:ring-0 rounded-none shadow-none" 
