...
async function sendEmail(payload: EmailPayload) {
  try {
    ...
    console.log("- Using domain:", "noreply@invite.medicdata.co.za");
    ...
    const htmlContent = `
      <html>
      <body>
        <h1>HealthForm Harvester Invitation</h1>
        <p>Hello,</p>
        ...
        <p>HealthForm Harvester &copy; ${new Date().getFullYear()}</p>
      </body>
      </html>
    `;
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "HealthForm Harvester <noreply@invite.medicdata.co.za>",
        to: [email],
